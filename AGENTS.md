...
- `js/engine.js`, `js/rtp.js`, and `js/bank.js` are intentionally **dependency-free** and **import nothing**. `rtp.js` does not import `engine.js`; instead the engine is passed in as a parameter (dependency injection) by `main.js` and the tests — e.g. `enumerateRTP(config, engine, k)`, `tune(config, engine)`, `simulate(config, engine, {...})`. All three modules must keep running in **both** the browser and Node: `engine.js` uses `globalThis.crypto` (Web Crypto, Node 18+) with an injectable/seeded RNG; `bank.js` falls back to an in-memory shim when `localStorage` is absent.
- **RTP as a function of the tuner knob `k` is U-shaped, not monotonic.** Flooding the reels with cheap-but-still-paying symbols first lowers RTP, then raises it again. `rtp.js#tune()` therefore scans a log grid (`config.tuner`), detects every target crossing, and bisects the preferred branch (default: first decreasing-branch crossing). It returns a diagnostics object (`success`, `actualRTP`, `tunedK`, `branch`, `bracket`, `crossings`, `report`, …); treat RTP as matched only when `success === true`. Do **not** "simplify" it back to a plain monotonic binary search — that diverges.
...
- The in-game debug panel (live RTP, tuned `k`, per-symbol contribution, "Simulate 1M spins") is gated behind the `?debug=1` query param **or** a long-press on the marquee (does not change the URL). Normal play does not show the panel.
- Phase 1 UI extras: **PAYTABLE** opens an accessible overlay built from `config.paytable`; **SOUND: ON/OFF** is a persisted mute preference via `bank.toggleMute()` (no audio yet).
